 ...
 (function () {
     ...
     ko.windowManager =
    {
        viewModel: function (configuration) {
            ...
            this.removeWindow = function (id) {
                var window = this.windows.remove(function (item) {
                    return item.id === id
                });
            };
        }
     ...
     ko.addTemplateSafe("windowButtonTemplate", "\
                    <div class=\"${cssClass}\" data-bind=\"click : clicked, attr: { title: title }, css : { right : isFirst(), left : isLast() }, hover : 'title-button-hover'\"><div class=\"title-button-inner\"><div class=\"icon ${ iconCssClass }\"></div></div></div>", templateEngine);
     ...
     ko.bindingHandlers.windowManager =
    {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            ...
            ko.renderTemplate("windowContainerTemplate", value,
            {
                templateEngine: templateEngine
            }, windowContainer, "replaceNode");
        },
    ...
    {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var $element = $(element), pinned = ko.utils.unwrapObservable(valueAccessor()), dragOptions =
            {
                addClasses: false,
                handle: '.title-bar',
        ...
        update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var $element = $(element), pinned = ko.utils.unwrapObservable(valueAccessor());
     ...
     ko.bindingHandlers.taskbarVisible =
    {
        'init': function (element, valueAccessor) {
            var $element = $(element), value = ko.utils.unwrapObservable(valueAccessor()), isCurrentlyVisible = element.style.display !== "none";
        ...
        'update': function (element, valueAccessor) {
            var $element = $(element), value = ko.utils.unwrapObservable(valueAccessor()), isCurrentlyVisible = element.style.display !== "none", width = $element.outerWidth(true), height = $element.outerHeight(true);
            ...
            if (value && !isCurrentlyVisible) {
                ...
                $element.fadeOut('slow', 'swing', function () {
                    $('<div></div>').width(width).height(height).css('display', 'inline-block').insertAfter($element).animate({
                        width: 0
                    }, 500, function () {
                        $(this).remove();
                    });
    ...
    {
        'init': function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var $element = $(element), show = ko.utils.unwrapObservable(valueAccessor());
            ...
            if (show) {
                ...
                $element.css({
                    'left': (-viewModel.width() * 2) + 'px'
                });
            }
        },
        'update': function (element, valueAccessor, allBindingsAccessor, viewModel) {
            var $element = $(element), show = ko.utils.unwrapObservable(valueAccessor()), visible = $element.offset().left > -viewModel.width(), $clone = $element.clone(false), taskbarPos = $('.' + viewModel.parentViewModel.taskbarCssClass()).offset(), buttonPos = $('.taskbar-item[data-id="' + viewModel.id + '"]').offset();

            if (show) {
                if (buttonPos && !visible) {
                    // maximise
                    $clone.width(55).height(45).css({
                        'opacity': 0,
                        'top': buttonPos.top + 'px',
                        'left': buttonPos.left + 'px'
                    }).appendTo('div.window-container .windows');
