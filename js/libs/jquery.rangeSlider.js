 ...
 (function($, undefined){
    ...
    $.widget("ui.rangeSlider", 
    {
        ...
        _create : function(){
            ...
            this.leftContainment = $("<div class='ui-rangeSlider-handleContainment ui-rangeSlider-leftContainment' />").css("position", "absolute").css("top", 0).css("left", 0);
            
            this.rightContainment = $("<div class='ui-rangeSlider-handleContainment ui-rangeSlider-rightContainment' />").css("top", 0).css("left", 0);
            
            this.leftHandle = $("<div class='ui-rangeSlider-handle  ui-rangeSlider-leftHandle' />").draggable({
                axis : "x",
            ...
            this.rightHandle = $("<div class='ui-rangeSlider-handle ui-rangeSlider-rightHandle' />").draggable({
                axis : "x",
            ...
            this.container = $("<div class='ui-rangeSlider-container' />").css("position", "absolute");
            
            this.bar = $("<div class='ui-rangeSlider-bar' />").draggable({
                axis : "x",
                drag : $.proxy(this._barMoved, this),
                stop : $.proxy(this._barStop, this),
                containment : this.container
            }).css("position", "absolute").css("top", 0).bind("mousewheel", $.proxy(this._wheelOnBar, this));
            
            this.arrows.left = $("<div class='ui-rangeSlider-arrow ui-rangeSlider-leftArrow' />").css("position", "absolute").css("left", 0).bind("mousedown", $.proxy(this._startScrollLeft, this));
            
            this.arrows.right = $("<div class='ui-rangeSlider-arrow ui-rangeSlider-rightArrow' />").css("position", "absolute").css("right", 0).bind("mousedown", $.proxy(this._startScrollRight, this));
            
            $(document).bind("mouseup", $.proxy(this._stopScroll, this));
            
            this.container.append(this.leftContainment).append(this.rightContainment).append(this.innerBar).append(this.bar).append(this.leftHandle).append(this.rightHandle);
            
            this.element.append(this.container).append(this.arrows.left).append(this.arrows.right).addClass("ui-rangeSlider").bind("mousewheel", $.proxy(this._wheelOnContainer, this));
        ...
        _initWidth : function(){
            ...
            this.leftContainment.css("width", this.container.innerWidth()).css("height", this.container.innerHeight());
            this.rightContainment.css("width", this.container.innerWidth()).css("height", this.container.innerHeight());
        },
        ...
        _position : function(){
            var leftPosition = this._getPosition(this._values.min, this.leftHandle), rightPosition = this._getPosition(this._values.max, this.rightHandle);
            
            this._positionHandles();
            this.bar.css("left", leftPosition).css("width", rightPosition - leftPosition + this.bar.width() - this.bar.outerWidth(true));
        },
        
        _positionHandles : function(){
            var left = this._getPosition(this._values.min, this.leftHandle), right = this._getPosition(this._values.max, this.rightHandle) - this.rightHandle.outerWidth(true), leftBounds = 
            {
            ...
            this.leftContainment.css("margin-left", left).css("width", Math.round(this._getLeftPosition(leftBounds.max, this.leftHandle) + this.leftHandle.outerWidth(true)) - left);
            
            left = Math.round(this._getLeftPosition(rightBounds.min, this.rightHandle));
            this.rightContainment.css("margin-left", left).css("width", Math.round(this._getLeftPosition(rightBounds.max, this.rightHandle) + this.rightHandle.outerWidth(true)) - left);
            this._positionLabels();
        },
        
        _barMoved : function(event, ui){
            var left = ui.position.left, right = left + this.bar.outerWidth(true);
        ...
        _switchHandles : function(){
            ...
            this.leftHandle.removeClass("ui-rangeSlider-rightHandle").addClass("ui-rangeSlider-leftHandle");
            this.rightHandle.addClass("ui-rangeSlider-rightHandle").removeClass("ui-rangeSlider-leftHandle");
        },
        
        _handleMoved : function(event, ui){
            var min = this._values.min, max = this._values.max;
        ...
        _setValues : function(min, max){
            var oldValues = this._values, difference = Math.abs(max - min);
        ...
        */
        _createLabel : function(label, whichOne){
            if(label === null){
                label = $("<div class='ui-rangeSlider-label'/>").addClass("ui-rangeSlider-" + whichOne + "Label").css("position", "absolute");
        ...
        _positionLabel : function(label, position){
            var topPos = this.leftHandle.offset().top - label.outerHeight(true), parent = label.offsetParent(), leftPos = position - parent.offset().left;
            
            topPos = topPos - parent.offset().top;
            
            label.css("left", leftPos).css("top", topPos);
        },
        ...
        _positionLabels : function(){
            if(this.labels.left !== null && this.labels.right !== null){
                this._fillInLabels();
                var minSize = this.labels.leftDisplayed ? this.labels.left.outerWidth(true) : 0, maxSize = this.labels.rightDisplayed ? this.labels.right.outerWidth(true) : 0, leftBound = 0, rightBound = $(window).width() - maxSize, minLeft = Math.max(leftBound, this.leftHandle.offset().left + this.leftHandle.outerWidth(true) / 2 - minSize / 2), maxLeft = Math.min(rightBound, this.rightHandle.offset().left + this.rightHandle.outerWidth(true) / 2 - maxSize / 2);
                ...
                if(this.labels.leftDisplayed)
                    this._positionLabel(this.labels.left, minLeft);
                if(this.labels.rightDisplayed)
                    this._positionLabel(this.labels.right, maxLeft);
            }
        },
        ...
        */
        ...
        bounds : function(min, max){
            if((typeof min !== "undefined") && (typeof max !== "undefined") && parseFloat(min) === min && parseFloat(max) === max && min < max){
                this.options.bounds = 
                {
                    min : min,
                    max : max
        ...
        zoomIn : function(quantity){
            var diff = this._values.max - this._values.min, min = this._values.min + quantity * this.options.wheelSpeed * diff / 200, max = this._values.max - quantity * this.options.wheelSpeed * diff / 200;
        ...
        scrollRight : function(quantity){
            ...
            var diff = this._values.max - this._values.min, min = this._values.min + quantity * this.options.wheelSpeed * diff / 100, max = this._values.max + quantity * this.options.wheelSpeed * diff / 100;
    ...
    $.widget("ui.dateRangeSlider", $.ui.rangeSlider, 
    {
        ...
        bounds : function(min, max){
            ...
            if(typeof min !== "undefined" && min instanceof Date && typeof max !== "undefined" && max instanceof Date){
