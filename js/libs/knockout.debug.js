 ...
 (function(window, undefined)
{
    ...
    ko.utils = new(function()
    {
        ...
        return {
            ...
            setTextContent: function(element, textContent)
            {
                ...
                'innerText'
                    in element ? element.innerText = value : element.textContent = value;
    ...
    ko.observableArray = function(initialValues)
    {
        ...
        if((initialValues !== null) && (initialValues !== undefined) && !('length'
        in initialValues))
    ...
    {
        var _latestValue,
        _hasBeenEvaluated = false, options = prepareOptions(evaluatorFunctionOrOptions, evaluatorFunctionTarget, options);
        ...
        function evaluateImmediate()
        {
            ...
            try
            {
                ...
                // If undefined, it will default to "window" by convention. This might change in the future.
    ...
    ko.exportSymbol('ko.jsonExpressionRewriting.insertPropertyAccessorsIntoJson', ko.jsonExpressionRewriting.insertPropertyAccessorsIntoJson); (function()
    {
    ...
    ko.exportSymbol('ko.bindingProvider', ko.bindingProvider); (function()
    {
        ...
        function applyBindingsToNodeInternal(node, bindings, viewModelOrBindingContext, isRootNodeForBindingContext)
        {
            ...
            var initPhase = 0;
            // 0 = before all inits, 1 = during inits, 2 = after all inits
            ...
            { (function()
                {
                    ...
                    // Separate variable to be captured by event handler closure
    ...
    ko.bindingHandlers['value'] = 
    {
        ...
        'update' : function(element, valueAccessor)
        {
            ...
            if((element.tagName == "SELECT") && (element.length > 0))
                ensureDropdownSelectionIsConsistentWithModelValue(element, newValue, 
            /* preferModelValue */
            false);
        }
    };
    ...
    ko.bindingHandlers['options'] = 
    {
        'update' : function(element, valueAccessor, allBindingsAccessor)
        {
            ...
            element.scrollTop = 0;
            // Workaround for a Chrome rendering bug. Note that we restore the scroll position later. (https://github.com/SteveSanderson/knockout/issues/215)
            ...
            if(value)
            {
                ...
                if(selectWasPreviouslyEmpty && ('value'
                in allBindings))
                {
                    // Ensure consistency between model value and selected option.
                    // If the dropdown is being populated for the first time here (or was otherwise previously empty),
                    // the dropdown selection state is meaningless, so we preserve the model value.
                    ensureDropdownSelectionIsConsistentWithModelValue(element, ko.utils.unwrapObservable(allBindings['value']), 
                    /* preferModelValue */
                    true);
                }
            }
    ...
    ko.bindingHandlers['checked'] = 
    {
        'init' : function(element, valueAccessor, allBindingsAccessor)
        {
            var updateHandler = function()
            {
                ...
                var modelValue = valueAccessor();
                if((element.type == "checkbox") && (ko.utils.unwrapObservable(modelValue)
                    instanceof Array))
                {
    ...
    })(); (function()
    {
        ...
        ko.activateBindingsOnTemplateRenderedNodes = function(nodeArray, bindingContext)
        {
            ...
            // So we can tolerate insertions/deletions during binding
            var commonParentElement = (nodeArray.length > 0) ? nodeArray[0].parentNode : null;
            // All items must be in the same parent, so this is OK
        ...
        ko.bindingHandlers['template'] = 
        {
            ...
            'update' : function(element, valueAccessor, allBindingsAccessor, viewModel, bindingContext)
            {
                ...
                if((typeof bindingValue === 'object') && ('foreach'
                in bindingValue))
                {
                    // Note: can't use 'in' operator on strings
                    // Render once for each data point (treating data set as empty if shouldDisplay==false)
                    var dataArray = (shouldDisplay && bindingValue['foreach']) || [];
                    templateSubscription = ko.renderTemplateForEach(templateName || element, dataArray, 
                    /* options: */
                    bindingValue, element, bindingContext);
                }
                else
                {
                    if(shouldDisplay)
                    {
                        ...
                        var innerBindingContext = (typeof bindingValue == 'object') && ('data'
                        in bindingValue) ? bindingContext['createChildContext'](ko.utils.unwrapObservable(bindingValue['data']))// Given an explitit 'data' value, we create a child binding context for it
                         : bindingContext;
                        // Given no explicit 'data' value, we retain the same binding context
                        templateSubscription = ko.renderTemplate(templateName || element, innerBindingContext, 
                        /* options: */
    ...
    ko.exportSymbol('ko.renderTemplate', ko.renderTemplate); (function()
    {
        ...
        function calculateEditDistanceMatrix(oldArray, newArray, maxAllowedDistance)
        {
            ...
            var oldIndex,
            oldIndexMax = oldArray.length, newIndex,
        ...
        function findEditScriptFromEditDistanceMatrix(editDistanceMatrix, oldArray, newArray)
        {
            ...
            if(maxDistance === undefined)
                return null;
            // maxAllowedDistance must be too small
